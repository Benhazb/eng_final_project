...
class PQEval(object):
    def __init__(self, Amax=1, Fs=48000, NF=2048, device=None, dtype=None):
        ...
        self.f = torch.linspace(0, torch.div(self.Fs, 2, rounding_mode='floor'),
                                torch.div(self.NF, 2, rounding_mode='floor') + 1, device=self.device, dtype=self.dtype)
        ...
        self.Bs = self.PQ_SpreadCB(torch.ones(self.Nc, device=self.device, dtype=self.dtype),
                                   torch.ones(self.Nc, device=self.device, dtype=self.dtype))

        # Allocate storage
        self.Eb = torch.zeros((2, self.Nc), device=self.device, dtype=self.dtype)
        self.Xw2 = torch.zeros((2, torch.div(self.NF, 2, rounding_mode='floor') + 1), device=self.device,
                               dtype=self.dtype)
        ...
        self.U = torch.zeros((torch.div(self.NF, 2, rounding_mode='floor') + 1, self.Nc), device=self.device,
                             dtype=self.dtype)

        for k in range(torch.div(self.NF, 2, rounding_mode='floor') + 1):
            for i in range(self.Nc):
                temp = (torch.amin(torch.stack([self.fu[i], (k + 0.5) * self.df])) - torch.amax(
                    torch.stack([self.fl[i], (k - 0.5) * self.df]))) / self.df
    ...
    def PQmovPD(self, EhsR, EhsT):
        ...
        cond = L > 0
        s = torch.where(cond, d1 * (d2 / L) ** g + c[0] + L * (c[1] + L * (c[2] + L * (c[3] + L * c[4]))),
                        1e30 * torch.ones_like(L))
...
class PEAQ(object):
    ...
    def process(self, referenceSignal, testSignal):
        ...
        self.X2MatR = torch.zeros((self.Np, torch.div(self.NF, 2, rounding_mode='floor') + 1), device=self.device,
                                  dtype=self.dtype)
        self.X2MatT = torch.zeros((self.Np, torch.div(self.NF, 2, rounding_mode='floor') + 1), device=self.device,
                                  dtype=self.dtype)
    ...
    def PQadapt(self, EhsR, EhsT, Mod='FFT'):
        ...
        M1, M2 = 3 * torch.ones((), dtype=torch.int, device=self.device), 4 * torch.ones((), dtype=torch.int,
                                                                                         device=self.device)
    ...
    def PQnNetB(self, MOV):
        ...
        amin, amax, wx, wxb, wy, wyb, bmin, bmax = list(
            map(lambda x: torch.tensor(x, device=self.device, dtype=self.dtype), output))
